...
def eval_class_dicts(gold_dict, pred_dict):
    avna = total = 0
    tp = tn = fp = fn = 0
    by_type = {}
    for key, value in pred_dict.items():
        ...
        if question_type not in by_type:
            by_type[question_type] = {'AvNA': 0.0, 'Total': 0}

        curr_avna = compute_avna(prediction, ground_truths)
        avna += curr_avna
        by_type[question_type]['AvNA'] += curr_avna
        by_type[question_type]['Total'] += 1

        tp += float(bool(prediction) == True and bool(ground_truths) == True)
        tn += float(bool(prediction) == False and bool(ground_truths) == False)
        fp += float(bool(prediction) == True and bool(ground_truths) == False)
        fn += float(bool(prediction) == False and bool(ground_truths) == True)


    for question_type in by_type:
        for metric in by_type[question_type]:
            if metric != 'Total':
                by_type[question_type][metric] =\
                    100. * by_type[question_type][metric] / by_type[question_type]['Total']

    eval_dict = {'AvNA': 100. * avna / total,
                 #'true_pos': 100. * tp / total,
                 #'true_neg': 100. * tn / total,
                 #'false_pos': 100. * fp / total,
                 #'false_neg': 100. * fn / total,
                 'precision': metric_frac(tp, fp),
                 'recall': metric_frac(tp, fn),
                 'negative_predictive_value': metric_frac(tn, fn),
                 'specificity': metric_frac(tn, fp),
...
def eval_dicts(gold_dict, pred_dict, no_answer):
    ...
    for key, value in pred_dict.items():
        ...
        if question_type not in by_type:
            by_type[question_type] = {'EM': 0.0, 'F1': 0.0, 'AvNA': 0.0, 'Total': 0}
        ...
        by_type[question_type]['EM'] += curr_em
        by_type[question_type]['F1'] += curr_f1
        by_type[question_type]['Total'] += 1
